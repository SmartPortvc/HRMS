...
import { Database, Search, Building, Briefcase, ChevronDown, Download } from 'lucide-react';
import { collection, query, getDocs, getDoc, doc as docFn, where } from 'firebase/firestore';
      ...
      const employeeData = await Promise.all(
        querySnapshot.docs.map(async (doc) => {
          ...
          if (data.organizationId && data.departmentId) {
            const deptRef = docFn(db, `organizations/${data.organizationId}/departments`, data.departmentId);
          ...
          const salaryQuery = query(salariesRef, where('userId', '==', data.uid));
  ...
  const exportToCSV = () => {
    try {
      ...
      const csvData = filteredEmployees.map(employee => [
        ...
        (employee.employmentType?.charAt(0).toUpperCase() + employee.employmentType?.slice(1)) || 'NA',
        employee.doj || 'NA',
        employee.ced || 'NA',
      ...
      link.setAttribute('download', `employee_master_data_${new Date().toISOString().split('T')[0]}.csv`);
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Division</th>
                <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Employment Type</th>
                <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Date of Joining</th>
                ...
                <tr key={employee.id} className="hover:bg-gray-50">
                  <td className="px-4 py-2 whitespace-nowrap">
                    <div>
                      ...
                      <div className="text-xs text-gray-500">{employee.email || 'N/A'}</div>
                      <div className="text-xs text-gray-500">{employee.designation || 'N/A'}</div>
                    </div>
                  </td>
                  <td className="px-4 py-2 whitespace-nowrap">
                    <div className="text-sm">
                      <div className="font-medium text-gray-900">{employee.departmentName || 'N/A'}</div>
                      <div className="text-xs text-gray-500">{employee.organizationName || 'N/A'}</div>
                    </div>
                  </td>
                  <td className="px-4 py-2 whitespace-nowrap">
                    <span className={`px-2 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full ${
                      employee.employmentType === 'regular' ? 'bg-green-100 text-green-800' :
                      employee.employmentType === 'contract' ? 'bg-yellow-100 text-yellow-800' :
                  ...
                  <td className="px-4 py-2 whitespace-nowrap text-xs">
                    <button
                      onClick={() => navigate(`/admin/master-data/employees/${employee.id}`)}
